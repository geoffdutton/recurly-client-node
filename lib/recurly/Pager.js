...
class Pager {
   ...
   */
  async count () {
    const empty = await this.client._makeRequest('HEAD', this.path, null, this._consumeParams())
  ...
  async first () {
    ...
    this.params.limit = 1
    const results = await this.client._makeRequest('GET', this.path, null, this._consumeParams())
  ...
  _resourceIterator () {
    const iterator = this._pageIterator()
